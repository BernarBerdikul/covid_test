...
from covidtest.sockets.middlewares import get_staff
    ...
    async def connect(self):
        ...
        try:
            ...
            if not email:
                await self.close()
            """ get params """
            self.scope["user"] = await get_staff(email=email)
            user_role: str = self.scope["user"].get_role
            """ set room """
            self.room_group_name: str = f"room_{user_role}"
            """ join room group """
